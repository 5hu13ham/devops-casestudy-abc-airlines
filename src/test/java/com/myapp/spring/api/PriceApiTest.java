...
public class PriceApiTest {
	...
	// Prepare Mock Flight
		...
		Price flight1 = new Price("AirAsia", "Kerala", "Kolkata", 3456.6, 4.5,Date.from(localDate1.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
		flight1.setFlightId(78);

		LocalDate localDate2 =LocalDate.of(2022, 12, 9);
		Price flight2 = new Price("Indigo","Delhi","Bangalore",2345.6,1.45,Date.from(localDate2.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
		...
		List<Price> flights = new ArrayList<>();
		flights.add(flight1);
		flights.add(flight2);
	...
	@Test
	@DisplayName("Test Get All Flights By  flightName,date /api/v1/flights/findByFlightNameAndDate")
	public void testGetAllFlightsByFlightNameAndDate() throws Exception {
	...
	// Prepare Mock Service Method
	String flightName="AirAsia";
	LocalDate localDate= LocalDate.of(2022,12,9);
	

	doReturn(flights).when(repository)
	.findByFlightNameAndDate(flightName,Date.from(localDate.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
	...
	.get("/api/v1/flights/findByFlightNameAndDate")
	.queryParam("flightName",flightName)
